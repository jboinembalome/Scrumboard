...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--blouppy-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--blouppy-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--blouppy-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ blouppy TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--blouppy-${e(key)}`, value], [`--blouppy-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`blouppy.customProps.background.${colorScheme}`);
            const foreground = theme(`blouppy.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--blouppy-${e(key)}`, value], [`--blouppy-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--blouppy-${e(key)}`, value], [`--blouppy-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
